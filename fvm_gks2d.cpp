...
//			int index = i * block.ny + j;
//
//			fluids[index].gama = 0.0;
//
//			double xzeta = max(xinterfaces[i * (block.ny + 1) + j].line.right.sensor, xinterfaces[i * (block.ny + 1) + j].line.left.sensor);
//			double yzeta = max(yinterfaces[i * (block.ny + 1) + j].line.right.sensor, yinterfaces[i * (block.ny + 1) + j].line.left.sensor);
//
//			fluids[index].sensor = max(xzeta, yzeta); //cell max
//		}
//	}
}
